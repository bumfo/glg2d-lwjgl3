...
class Lwjgl3GL20 implements org.lwjgl.glg2d.bridge.GL20 {
  ...
  private void ensureBufferCapacity(int numBytes) {
    if (buffer == null || buffer.capacity() < numBytes) {
      buffer = org.lwjgl.glg2d.bridge.BufferUtils.newByteBuffer(numBytes);
  ...
  public void glDrawElements(int mode, int count, int type, Buffer indices) {
    if (indices instanceof ShortBuffer && type == org.lwjgl.glg2d.bridge.GL20.GL_UNSIGNED_SHORT) {
      GL11.glDrawElements(mode, (ShortBuffer) indices);
    } else if (indices instanceof ByteBuffer && type == org.lwjgl.glg2d.bridge.GL20.GL_UNSIGNED_SHORT) {
      GL11.glDrawElements(mode, ((ByteBuffer) indices).asShortBuffer());
    } else if (indices instanceof ByteBuffer && type == org.lwjgl.glg2d.bridge.GL20.GL_UNSIGNED_BYTE) {
  ...
  public void glGetFloatv(int pname, float[] data, int data_offset) {
    if (data_offset != 0) throw new UnsupportedOperationException("data_offset must be 0");
